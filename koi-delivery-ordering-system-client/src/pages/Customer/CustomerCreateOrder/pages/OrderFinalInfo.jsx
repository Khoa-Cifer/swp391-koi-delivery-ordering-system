...
function OrderFinalInfo({ orderId }) {
  ...
  async function handlePostOrder() {
    const paymentResponse = await logPaymentHistory(
      customerId,
      orderId,
      Math.floor(postedData.price)
    );
    const paymentOpen = await paymentOpenGateway(
      customerId,
      Math.floor(postedData.price),
      "NCB"
    );
    ...
    <Box>
      ...
      {postedData && (
        ...
        >
          ...
          <Grid container spacing={2}>
            {fishOrderData &&
              fishOrderData.length > 0 &&
              fishOrderData.map((fish, index) => (
          ...
          >
            {paymentSuccess ? (
              ...
              <SubmitButton variant="contained" onClick={handlePostOrder}>
                Post as Order
              </SubmitButton>
            )}
          </Box>
