...
import com.swp391team3.koi_delivery_ordering_system.service.TokenService;
    ...
    public ResponseEntity<?> login(@RequestBody UserRequestLoginDTO request) {
        ...
        if (userType == 1) {
            ...
            if (status) {
                ...
                response = new UserResponseLoginDTO(foundCustomer.getEmail(), foundCustomer.getUsername(), "Customer", userType, tokenService.generateToken(foundCustomer));
            }
        } else if (userType == 2) {
            ...
            if (status) {
                ...
                response = new UserResponseLoginDTO(foundSalesStaff.getEmail(), foundSalesStaff.getUsername(), "Delivery Staff", userType, tokenService.generateToken(foundSalesStaff));
            }
        } else if (userType == 3) {
            ...
            if (status) {
                ...
                response = new UserResponseLoginDTO(foundDeliveryStaff.getEmail(), foundDeliveryStaff.getUsername(), "Sales Staff", userType, tokenService.generateToken(foundDeliveryStaff));
            }
        }
